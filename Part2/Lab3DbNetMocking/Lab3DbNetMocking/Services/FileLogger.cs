...
namespace Lab3DbNet.Core.Services
{
    public class FileLogger : ILogger
    {
        ...
        public async Task Log(string message)
        {
            try
            {
                ...
                await File.AppendAllTextAsync(this._logFilePath, $"{DateTime.Now}: {message}{Environment.NewLine}");
            }
            catch (Exception ex)
            {
        ...
        public async Task LogError(string message, Exception ex)
        {
            try
            {
                ...
                await File.AppendAllTextAsync(this._logFilePath, $"{DateTime.Now}: ERROR - {message} - Exception: {ex}{Environment.NewLine}");
            }
